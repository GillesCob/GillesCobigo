      ...
      <button onClick={onToggle} className="w-full text-left group">
        <div className={`flex items-start gap-2 ${alignRight ? 'flex-row-reverse' : 'flex-row'}`}>
          <div className={`flex-1 ${alignRight ? 'text-right' : 'text-left'}`}>
            <p className="font-semibold text-base group-hover:text-foreground transition-colors">
              {item.title}
            </p>
    ...
    <section className="py-16 px-4 border-b border-border">
      <div className="max-w-4xl mx-auto">
        ...
        <p className="text-muted-foreground mb-12">
          10 ans de BIM, 3 ans de code. Pas une reconversion - une extension.
        </p>
                ...
                >
                  ...
                  <div className="hidden md:flex items-start">
                    <div className="flex-1 pr-10">
                      {isEven ? (
                        <p className="text-right font-mono text-sm text-muted-foreground/60 pt-1">
                          {item.year}
                        </p>
                      ) : (
                        <TimelineCard
                    ...
                    <div className="flex-1 pl-10">
                      {isEven ? (
                        ...
                        <p className="font-mono text-sm text-muted-foreground/60 pt-1">
                          {item.year}
                        </p>
                      )}
                    </div>
