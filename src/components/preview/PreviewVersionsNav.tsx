...
interface IPreviewVersionsNavProps {
  slug: string;
  secret: string;
  currentRound: string;
...
export default function PreviewVersionsNav({ slug, secret, currentRound, rounds }: IPreviewVersionsNavProps) {
    ...
    <>
      ...
      <aside className="hidden md:flex flex-col w-40 flex-shrink-0 gap-1">
        {rounds.map((r) => (
          <Link
            key={r.round}
            to={`/preview/${slug}/${secret}/${r.round.toLowerCase()}`}
